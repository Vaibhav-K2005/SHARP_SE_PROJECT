...
automatically, laundry batches are tracked by reusable QR identity, and mess feedback stays
anonymous to mess staff while remaining auditable.
\end{abstract}

\begin{IEEEkeywords}
hostel management, workflow automation, role-based access control, SLA escalation,
QR tracking, web application
\end{IEEEkeywords}

\section{Introduction}
Several routine hostel processes are partially manual or lack tracking, so requests move
through paper, phone calls and registers without an auditable trail: leave approvals are
delayed, complaint status is unknown, clusters break under staggered fee payment, and laundry
is lost without recourse. SHARP gives every participant one platform for the services their
role covers, and each module adds a mechanism the manual process lacks---a verification step,
an expiry rule or an escalation timer---rather than reproducing paperwork on a screen.
...
and preferred slots go unhonoured. \textit{Laundry:} manual slips leave no record of batches,
so losses stay invisible. Each is a traceability gap.
...
Fig.~\ref{fig:complaint}; what distinguishes it from a status field is student verification
before closure and SLA escalation to the warden. \textit{Laundry management} replaces permanent
markings on clothing with a reusable QR-coded bag bound to the student's account: request
$\rightarrow$ scan $\rightarrow$ items received $\rightarrow$ washing $\rightarrow$ ready
$\rightarrow$ collection $\rightarrow$ confirmation. Counts are recorded by category on
receipt, the code is rescanned on return, and reports of missing or damaged items become
statistics that expose recurring failures.
...
\node[mod, right=of m4] (m5) {Laundry (QR)};
...
\node[lay, below=10mm of m4] (d1) {Users, requests,\\complaints, batches};
...
\node[rectangle, rounded corners=2pt, draw=black!70, fill=black!6, font=\scriptsize,
      ...
      text width=70mm] (x)
  {\textbf{Optional external services:} email, SMS or push notifications; QR library};
...
Express, MongoDB, token authentication and a QR library---is a proven candidate and is used
for costing, but any stack offering a REST-capable framework, a managed database, secure token
authentication and a test framework satisfies the design. Deferring the choice costs nothing:
no workflow rule, role definition or metric depends on a particular language.

\section{Requirements}
Students file leave requests, track approvals, hold passes, submit anonymous feedback and meal
requests, raise and reopen complaints with preferred slots, and track laundry. Parents act on
leave notifications; wardens approve leave, manage rooms and clusters, assign caretakers and
view analytics; caretakers accept assignments, propose slots and update status; mess and
laundry staff work only within their own queues. Non-functionally, operations should complete
within seconds at hostel-scale load, passwords must be hashed, parent approval tokens must
resist unauthorised use, the service must stay available outside administrative hours, and the
schema should admit further hostels without redesign.

\section{Feasibility and Cost}
The system needs no special hardware, since QR scanning uses phone cameras, and the toolchain
is free or open source: the prototype costs roughly \textrm{INR}\,0--3{,}500, dominated by
optional hosting, domain, SMS and QR cards. It is operationally feasible because it replaces
workflows students already understand. Delivery is phased---authentication and roles, then
leave, complaints, mess, allotment, laundry, and finally notifications and deployment---so a
working minimum viable product exists even if later phases are unfinished.
...
transactional validation; lost laundry, met by batch tracking and collection confirmation; and
server failure, met by backup and recovery. Success is measured by the fall in paper
applications, average leave approval and complaint resolution times, share of complaints closed
within SLA, reopen count, laundry complaints per hundred batches and reservation conflicts
avoided---all computable from the platform's own records.

\section{Conclusion}
SHARP turns five disconnected hostel processes into one auditable platform, adding
verification, expiry and escalation mechanisms rather than merely replacing paper. It is
feasible technically, economically and operationally, with a mobile application, predictive
maintenance and multi-campus support as future scope.
