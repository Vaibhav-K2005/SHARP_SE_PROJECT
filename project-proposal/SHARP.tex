...
\begin{itemize}[leftmargin=0.7cm]
    ...
    early.
    \item We will prioritize fast feedback over perfection by phasing delivery (leave,
...
expiry rule, an SLA timer) can noticeably reduce unresolved cases and disputes.
...
\begin{itemize}[leftmargin=0.7cm]
    ...
    assignment, and closure only after student verification, with SLA escalation to the
    warden.
...
\begin{enumerate}[leftmargin=0.7cm]
    ...
    an SLA timer runs independently and notifies the warden if breached.
...
This is an engineering course project, so we focus on layered architecture, role-based access
...
\begin{itemize}[leftmargin=0.7cm]
    ...
    \item \textbf{Business logic:} the five modules (leave \& pass, mess feedback, allotment,
    complaints \& SLA, laundry issues) plus notifications and analytics.
...
\begin{itemize}[leftmargin=0.7cm]
    ...
    closure.
    \item \textbf{SLA compliance:} share of complaints closed within the defined SLA window.
...
\begin{itemize}[leftmargin=0.7cm]
    \item \textbf{Paper reduction:} fall in paper-based leave applications after rollout.
...
\begin{itemize}[leftmargin=0.7cm]
    ...
    \item Leave and visitor pass workflow: Student $\rightarrow$ Parent $\rightarrow$ Warden
...
\begin{itemize}[leftmargin=0.7cm]
    ...
    submission.
    \item \textbf{Staff adoption:} keep workflow steps minimal for caretakers, mess and
    laundry staff, and provide an easy UI for status updates.
\end{itemize}

\section{Summary}
This project proposes a deployable web platform, SHARP, that turns five disconnected hostel
processes (leave and passes, mess feedback, allotment, maintenance complaints and laundry)
into one auditable system. It meets the course criteria by emphasizing time-to-value through
phased delivery, implementing CI/CD to support frequent safe releases, and using measurable
evaluation criteria such as complaint resolution time and SLA compliance. It remains focused on
engineering workflow correctness, traceability and scalability readiness rather than
research-driven novelty, and is feasible technically, economically and operationally as a
course project (prototype cost roughly \textrm{INR}\,0--3{,}500, dominated by optional hosting,
domain and SMS).
